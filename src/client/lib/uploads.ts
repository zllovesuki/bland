...
export async function uploadFile(workspaceId: string, file: File, pageId?: string): Promise<string> {
  ...
  const upload = await api.uploads.presign(workspaceId, {
    filename: file.name,
    content_type: file.type as PresignRequest["content_type"],
    size_bytes: file.size,
    page_id: pageId ?? null,
  });
  await api.uploads.uploadData(upload.upload_url, file);
  return upload.url;
}
